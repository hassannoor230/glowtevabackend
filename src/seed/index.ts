...
const seed = async () => {
  try {
    ...
    const existingDemoUser = await User.findOne({ email: 'elena@example.com' });
    if (!existingDemoUser) {
    ...
    const insertedMain = missingMainCategories.length > 0
      ? await Category.insertMany(missingMainCategories)
      : [];
    ...
    const insertedSubs = missingSubCategories.length > 0
      ? await Category.insertMany(missingSubCategories)
      : [];
    ...
    const existingProductSlugs = new Set(
      (await Product.find({ slug: { $in: products.map(product => product.slug) } })
        .select('slug'))
        .map(product => product.slug),
    );
    const productsToCreate = products.filter(product => !existingProductSlugs.has(product.slug));
    if (productsToCreate.length > 0) {
      await Product.insertMany(productsToCreate);
    }
    console.log(`Created ${productsToCreate.length} products`);

    const existingJournalSlugs = new Set(
      (await Journal.find({ slug: { $in: journals.map(journal => journal.slug) } })
        .select('slug'))
    ...
    if (journalsToCreate.length > 0) {
      await Journal.insertMany(journalsToCreate);
    }
    ...
    const existingCouponCodes = new Set(
      (await Coupon.find({ code: { $in: couponData.map(coupon => coupon.code) } })
        .select('code'))
        .map(coupon => coupon.code),
    );
    const couponsToCreate = couponData.filter(coupon => !existingCouponCodes.has(coupon.code));
    if (couponsToCreate.length > 0) {
      await Coupon.create(couponsToCreate);
    }
